...
const detailFields = computed(() => templateFields.value.filter(field => field.key !== 'sets'))
...
function goToPage2() {
  ...
  setDetails.value = Array.from({ length: formValues.value.sets }, (_, i) => {
    ...
    detailFields.value.forEach(field => {
      ;(detail as Partial<TemplateValues>)[field.key] = formValues.value[field.key]
    })
    return detail
  })
...
<template>
  <van-popup
    ...
    position="bottom"
    round
    :style="{ width: 'calc(100% - 16px)', left: '8px', bottom: '8px', height: '80%', borderRadius: '24px', background: 'transparent', '--van-ease-out': 'cubic-bezier(0.16, 1, 0.3, 1)', '--van-ease-in': 'cubic-bezier(0.16, 1, 0.3, 1)' }"
    @update:show="emit('update:show', $event)"
  >
    <div class="drawer-container" :class="{ 'nested-drawer-open': nestedEditorOpen }">
      <!-- 页面切换容器 -->
      <div class="pages-wrapper" :style="{ transform: `translateX(-${currentPage * 50}%)` }">
        <!-- 第一页：选择动作和组数 -->
        <div class="page page-1">
          ...
          <footer class="drawer-footer">
            ...
            <button class="btn btn-primary" @click="goToPage2">
              确认
        ...
        <div class="page page-2">
          ...
          <SetDetailsEditor :sets="setDetails" :fields="templateFields" @update="updateSet" @delete="deleteSet" @editor-visibility="onNestedEditorVisibility" />

          <footer class="drawer-footer">
            <button class="btn btn-secondary" @click="goToPage1">
              <span class="btn-arrow">←</span>
              上一页
            </button>
