...
export async function generateWorkoutShareImage(
  ...
  context.font = '700 64px -apple-system, BlinkMacSystemFont, "PingFang SC", sans-serif'
  context.fillText(options.title, CARD_MARGIN, 184)

  const setCount = options.items.reduce((total, item) => total + item.sets.length, 0)
  ...
  for (const item of options.items) {
    ...
    fillRoundRect(context, CARD_MARGIN, cardY, cardWidth, cardHeight, 34, '#ffffff')
    context.restore()

    fillRoundRect(context, CARD_MARGIN + 28, cardY + 28, 58, 58, 18, '#edf5ff')
    context.fillStyle = '#007aff'
    context.font = '700 26px -apple-system, BlinkMacSystemFont, "PingFang SC", sans-serif'
    context.textAlign = 'center'
    context.fillText(item.exerciseName.slice(0, 1), CARD_MARGIN + 57, cardY + 57)
    context.textAlign = 'left'

    context.fillStyle = '#1c1c1e'
    context.font = '650 34px -apple-system, BlinkMacSystemFont, "PingFang SC", sans-serif'
    context.fillText(
      fitText(context, item.exerciseName, cardWidth - 250),
      CARD_MARGIN + 108,
      cardY + 57,
    )
  ...
  context.fillStyle = '#1c1c1e'
  context.font = '650 30px -apple-system, BlinkMacSystemFont, "PingFang SC", sans-serif'
  context.fillText('扫码打开训练记录', CARD_MARGIN + 218, footerY + 60)
  context.fillStyle = '#8e8e93'
  context.font = '400 27px -apple-system, BlinkMacSystemFont, "PingFang SC", sans-serif'
